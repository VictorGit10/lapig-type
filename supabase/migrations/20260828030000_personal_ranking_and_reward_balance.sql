...
as $$
  with user_best as (
    select distinct on (r.user_id)
      r.user_id as ranked_user_id,
      r.gross_wpm,
      r.accuracy,
      r.score,
      r.created_at
    from public.results r
    where r.trust_status = 'accepted'
    order by r.user_id, r.score desc, r.accuracy desc, r.gross_wpm desc, r.created_at asc
  ), ranked as (
    select
      ub.*,
      row_number() over (
        order by ub.score desc, ub.accuracy desc, ub.gross_wpm desc, ub.created_at asc, ub.ranked_user_id
      ) as placement
    from user_best ub
  )
  select
    ...
    ranked.placement
  from ranked
  where ranked.placement <= least(greatest(coalesce(p_limit, 20), 1), 100)
    or (p_user_id is not null and ranked.ranked_user_id = p_user_id)
  order by ranked.placement;
$$;

comment on function public.get_typing_leaderboard(uuid, integer) is
  'Public top of the ranking plus, when p_user_id is supplied, that player own row with the real placement even outside the cut.';

revoke all on function public.get_typing_leaderboard(uuid, integer) from public, anon, authenticated;
grant execute on function public.get_typing_leaderboard(uuid, integer) to service_role;
