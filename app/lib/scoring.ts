...
export function scoreAttempt(input: ScoreInput): ScoreVerdict {
  ...
  if (!Number.isInteger(input.expectedChars) || input.expectedChars < 80 || input.expectedChars > 1200) fatal.push('invalid_expected_length');
  if (events.length > 2400 || events.length < input.expectedChars) fatal.push('invalid_event_count');
  if (correctEvents.length !== input.expectedChars) fatal.push('incomplete_sequence');
  ...
  const grossWpm = Math.round((input.expectedChars / 5) / Math.max(durationMs / 60_000, 1 / 60_000));
  const accuracy = Math.round((input.expectedChars / Math.max(events.length, 1)) * 100);
